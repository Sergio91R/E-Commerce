...
describe('CheckoutService - validaciones y edge cases', () => {
  ...
  it('el mensaje de stock insuficiente incluye el NOMBRE del producto, no su id interno', () => {
    const { service } = buildService();
    try {
      service.checkout([{ productId: 'p1', quantity: 3 }], undefined);
      fail('Se esperaba que lanzara InsufficientStockError');
    } catch (error) {
      expect((error as Error).message).toContain('Laptop');
